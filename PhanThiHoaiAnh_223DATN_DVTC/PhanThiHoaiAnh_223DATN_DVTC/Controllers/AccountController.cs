...
namespace PhanThiHoaiAnh_223DATN_DVTC.Controllers
{
	public class AccountController : Controller
	{
		...
		public async Task<IActionResult> Create(UserModel user)
		{
			if (ModelState.IsValid)
			{
				AppUserModel newUser =  new AppUserModel {Name=user.Name,UserName=user.UserName,Email=user.Email,Birthday=user.Birth,Gender=user.Gender,PhoneNumber=user.PhoneNumber,Address=user.Address};
